...
- **Invite flow**: Invites are **optional** for registration. Users can sign up freely. Invite codes can be used to track referrals and onboard specific users, but are not required. Two invite paths: (1) **Send Email Invite** — enter email, backend sends invite email via Resend, single-use; (2) **Generate Shareable Link** — create multi-use link to share manually. Both options available from pod detail, session detail, and standalone invite page.
