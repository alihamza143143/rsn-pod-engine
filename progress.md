...
### 2026-03-08 11:45 PM - Entry [Latest]
- Task ID: T-AUTH-003
- Task Title: Google OAuth env wiring for local + deployment
- Status: Completed
- What changed:
  - Added Google OAuth placeholders to server/.env.example
  - Added GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to ender.yaml env var list (sync false)
  - Added provided Google OAuth credentials to local server/.env (gitignored, local only)
- Files touched:
  - server/.env.example
  - render.yaml
  - server/.env (local, not tracked)
- Decisions made:
  - Keep secrets out of git; only local .env stores real credentials
  - Deployment secrets must be set via Render dashboard env vars
- Next immediate action:
  - Update Google Console authorized redirect URIs to backend callback URL(s)
  - Set production GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in Render service settings
